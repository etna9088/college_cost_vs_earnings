...
* **Predictive Earnings Model:** Utilized linear regression to map the exact correlation between tuition and income, revealing that every $1 increase in attendance cost yields an approximate $4.59 increase in median earnings.
* **Variance Evaluation:** Calculated an R-squared value of 34.4%, proving mathematically that while tuition cost is a strong indicator of future earnings, other external variables (like major, location, and industry) play a heavier role in total income variation.
...
Raw data was extracted from the publicly available U.S. Department of Education College Scorecard dataset. The data was loaded into RStudio, where it underwent extensive wrangling to handle missing values and isolate the target variables (`COSTT4_A` and `MD_EARN_WNE_P10`). The clean data was then modeled using standard regression algorithms to identify statistical significance and generate residual diagnostics.
